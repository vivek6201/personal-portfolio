...
export default function Skill({directionLeft,skill}: Props) {
  return (
    ...
    className='group relative flex cursor-pointer flex-shrink-0'>
      <motion.div className='rounded-full border overflow-hidden flex border-gray-500 w-20 aspect-square filter group-hover:grayscale md:w-24'>
        <Image src={urlFor(skill.image).url()} alt='' width={200} height={200} className='w-full h-full object-cover'/>
      </motion.div>
